...
完整构建默认严格抓取：任一已启用规则源下载失败、返回 HTML、没有有效规则或包含非法规则时停止。构建、产物校验与测试均在临时副本执行，全部通过后才替换发布文件；测试不得改写产物。银行补充源失败时可保留已验证的旧规则并输出警告。

任一分类规则数比构建前减少超过 20% 时默认阻止发布。确认是正常上游删除或分类调整后，才使用 `python build.py --allow-large-drop`，或手动运行更新工作流时选择同名选项；空规则文件始终拒绝。

PR 检查离线验证已发布文件并运行隔离测试，不抓取上游。日常更新任务串行运行；同一组输入没有变化时不创建提交。Clash/Mihomo 的 `.list` 为纯文本 classical 规则，引用方需设置 `behavior: classical` 和 `format: text`。
